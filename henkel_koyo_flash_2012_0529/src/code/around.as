package code
{
	...
	public class  around extends MovieClip {
		...
		{
			arrTextFrame = [pr1, pr2, pr3, pr4, pr5, pr6];
			arrProFrame = [pro1, pro2, pro3, pro4, pro5, pro6];
			...
			//re1.addEventListener(Event.CHANGE, onChangeHandle);
		...
		private function onResultHandle(e:MouseEvent):void {
			...
			//resFinal.text = String(Number(pre4.text) * Number(txtPic.text) * Number(txtDay.text));
		...
		private function setRes(str:String):void {
			...
			switch(str) {
				case "1":
					...
					rot =((Number(pro4.getNum()) - Number(pro1.getNum())) / Number(pro1.getNum())) * 360;
					subTime1.setRot(rot);
					break;
				case "2":
					getRes(re2, Number(pr2.text), pro2.getNum());
					
					rot =((Number(pro5.getNum()) - Number(pro2.getNum())) / Number(pro2.getNum())) * 360;
					subTime2.setRot(rot);
					break;
				case "3":
					getRes(re3, Number(pr3.text), pro3.getNum());
					
					rot =((Number(pro6.getNum()) - Number(pro3.getNum())) / Number(pro3.getNum())) * 360;
					subTime3.setRot(rot);
					break;
				case "4":
			...
			res1.text = String(Math.round((Number(re1.text) + Number(re2.text) + Number(re3.text))*100000)/100000);
			res2.text = String(Math.round((Number(re4.text) + Number(re5.text) + Number(re6.text))*100000)/100000);
